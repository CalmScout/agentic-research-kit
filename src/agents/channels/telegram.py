...
class TelegramChannel(Channel):
    ...
    def __init__(self):
        ...
        self._on_message_callback: Optional[Callable[[InboundMessage], Awaitable[None]]] = None
        self._running = False
    ...
    async def start(self, on_message: Callable[[InboundMessage], Awaitable[None]]):
        ...
        if self.settings.telegram_proxy:
            builder = builder.proxy(self.settings.telegram_proxy).get_updates_proxy(self.settings.telegram_proxy)
        
        self._app = builder.build()
        
        # Add handlers
        self._app.add_handler(CommandHandler("start", self._on_start))
        self._app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, self._handle_telegram_message))
