...
async def evaluate_retrieval(
    ...
    for idx, row in ground_truth.iterrows():
        ...
        try:
            ...
            is_retrieved = any(
                reviewed_claim.lower() in text.lower() for text in source_texts
            )
    ...
    metrics["mrr"] = (
        sum(results["mrr"]) / len(results["mrr"]) if results["mrr"] else 0.0
    )
    metrics["success_rate"] = results["successful_queries"] / results["total_queries"]

    # Include raw counts for display
    metrics["total_queries"] = results["total_queries"]
    metrics["successful_queries"] = results["successful_queries"]
