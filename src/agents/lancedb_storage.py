    ...
    async def initialize(self):
        ...
        async with self._storage_lock:
            ...
            try:
                ...
                self._table = None
                logger.info(
                    f"[{self.workspace}] LanceDB table {self.table_name} will be created on first use"
                )
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        async with self._storage_lock:
            ...
            for k, v in data.items():
                ...
                upsert_data.append({"id": k, "data": json.dumps(v)})

            if self._table is None:
                schema = pa.schema([pa.field("id", pa.string()), pa.field("data", pa.string())])
                self._table = self._db.create_table(
                    self.table_name, data=upsert_data, schema=schema
                )
    ...
    async def is_empty(self) -> bool:
        ...
        async with self._storage_lock:
            if self._table is None:
                return True
            try:
                return cast(bool, self._table.count_rows() == 0)
            except Exception:
                return True
...
class LanceDBDocStatusStorage(DocStatusStorage):
    ...
    async def get_by_ids(self, ids: list[str]) -> list[dict[str, Any]]:
        ...
        async with self._storage_lock:
            if self._table is None:
                # Return list of empty dicts or however LightRAG expects it
                return [{} for _ in ids]
            ...
            try:
                ...
                for id in ids:
                    val = lookup.get(id)
                    ordered_results.append(val if val is not None else {})
            except Exception:
                ordered_results = [{} for _ in ids]
        return cast(list[dict[str, Any]], ordered_results)
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        async with self._storage_lock:
            ...
            for doc_id, doc_data in data.items():
                ...
                upsert_data.append({"id": doc_id, "data": json.dumps(doc_data)})
    ...
    async def get_by_id(self, id: str) -> dict[str, Any] | None:
        async with self._storage_lock:
            ...
            try:
                results = self._table.search().where(f"id = '{id}'").to_pandas()
                if not results.empty:
                    return cast(dict[str, Any], json.loads(results.iloc[0]["data"]))
            except Exception:
                pass
            return None
...
class LanceDBVectorDBStorage(BaseVectorStorage):
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        for i, (k, v) in enumerate(data.items()):
            ...
            upsert_data.append(
                {"id": k, "vector": embeddings[i].tolist(), "metadata": json.dumps(metadata)}
            )
    ...
    async def get_vectors_by_ids(self, ids: list[str]) -> dict[str, list[float]]:
        async with self._storage_lock:
            ...
            return cast(
                dict[str, list[float]],
                {row["id"]: row["vector"].tolist() for _, row in df_filtered.iterrows()},
            )
